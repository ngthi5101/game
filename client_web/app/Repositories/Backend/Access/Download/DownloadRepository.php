...
namespace App\Repositories\Backend\Access\Download;
...
class DownloadRepository extends BaseRepository
{
     ...
     */
    public function update( $download,  array $input)
    {
        $download->ios = $input['ios'];
        $download->android = $input['android'];
        $download->window = $input['window'];
        $download->macos = $input['macos'];
        $download->save();
    }
    ...
    protected function createPost($input)
    {
        ...
        $post->body = bcrypt($input['body']);
        $post->slug = bcrypt($input['slug']);
        return $post;
    }
