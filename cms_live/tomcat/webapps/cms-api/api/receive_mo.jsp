...
<%!
	...
	public void playerRegistered(Connection connection,long id,String msisdn,String nickName) throws Exception
	{
		...
		String sql =
			...
			"BEGIN\n" +
			"	SELECT player_id INTO v_master_id FROM player WHERE nick_name = 'thanbai888.com';\n" +
			"	FOR r IN c LOOP\n" +
			...
			"					   'thanbai888.com: ban duoc tang ' || v_amount || ' phinh vi da moi '\n" +
			"					|| v_nick_name || ' choi game. Cam on ban rat nhieu.');\n" +
			"		ELSE\n" +
			"			INSERT INTO pm (pm_id, sender, recipient, content)\n" +
			"			VALUES (seq_pm.NEXTVAL, v_master_id, r.player_id,\n" +
			"					   'thanbai888.com: ' || v_nick_name\n" +
			"					|| ' da nhan loi moi tu mot nguoi choi khac. Moi ban tiep tuc choi va ung ho thanbai888.com.');\n" +
			"		END IF;\n" +
			"	END LOOP;\n" +
			...
			String sender, String recipient, String content) throws Exception
	{
		...
		try
		{
			...
			if(params.length < 2)
			{
				...
				return "link tai thanbai888.com - " + downloadLink;
			}
				...
				if(!PlayerManager.getInstance().isPlayerExists(cn, nickName))
				{
					...
					if(nicks != null && !nicks.isEmpty())
						throw new AppException(
							"thanbai888.com: Tai khoan " + nickName + " khong ton tai tren he thong.\n" +
...
<%
	...
	partnerSignKeysByPartner.put("VSII", "rBz8hXh2TTTSw12cst8C4qhImfpTvRUn");
	partnerSignKeysByPartner.put("HALINH", "rBz8hXh2TTTSw12cst8C4qhImfpTvHLn");
	partnerSignKeysByPartner.put("CASINOVIET", "rBz8hXh2TTTSwQScst8C4qhImfpTvRUn");
	partnerSignKeysByPartner.put("VCCORP", "yr3dw8meatrosj4g86bzpckcgro6tcfr4");
	partnerSignKeysByPartner.put("VCCORP0", "yr3dw8meatrosj4g86bzpckcgro6tcfr4");
	partnerSignKeysByPartner.put("VCCORP1", "yr3dw8meatrosj4g86bzpckcgro6tcfr4");
	partnerSignKeysByPartner.put("VCCORP2", "yr3dw8meatrosj4g86bzpckcgro6tcfr4");
	partnerSignKeysByPartner.put("VCCORP3", "yr3dw8meatrosj4g86bzpckcgro6tcfr4");
	partnerSignKeysByPartner.put("MVAS", "QF4W5mb569M8ezbMyhjuBq08PbQhHsCWG");
	partnerSignKeysByPartner.put("NHATQUANG", "QcOAongipfw41opbUqLWjdtB2z42cA6RO");
	...
	try
	{
		...
		String partnerSignKey = partnerSignKeysByPartner.get(partner == null || partner.equals("") ? "CASINOVIET" : partner);
		if (partnerSignKey == null)
			throw new Exception("INVALID_PARTNER");
		...
		listParams.add(partnerSignKey);
		listParams.add(sender);
		listParams.add(recipient);
		listParams.add(message);
		if (time != null && !time.equals(""))
			listParams.add(time);
		if(partner != null && !partner.equals(""))
			listParams.add(partner);

		String signSource = StringUtil.join(listParams, ":");
		String ourSign = MD5.hash(signSource);
                System.out.println("signkey_______________"+ourSign);
		if(!ourSign.equals(signKey))
			throw new Exception("INVALID_SIGNKEY");

		Map result = new HashMap();
		result.put("Message",receivedMO(sender, recipient, message));
