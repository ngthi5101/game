...
namespace App\Repositories\Backend\Access\Post;
...
class PostRepository extends BaseRepository
{
    ...
    public function update(Model $user, array $input, $filename)
    {
        ...
        if($filename != null){
            $user->img_feature = "/uploads/" . $filename;
        }
        $user->body = $input['tinymce_box'];
        $user->cate_id = $input['cate'];
        $user->slug = createSlug($input['title']);
        $user->save();
